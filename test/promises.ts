...
const canInspectPromises = typeof Promise !== "undefined" && Promise.toString().indexOf("[native code]") !== -1;

describe("promises", () => {
  describe("default behaviour", () => {
    beforeEach(function () {
      if (canInspectPromises) {
        this.skip();
      }
    });
    ...
    describe("truncate", () => {
      it("returns the full string representation regardless of truncate", () => {
        expect(inspect(Promise.resolve(), { truncate: 9 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 8 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 7 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 6 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 5 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 4 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 3 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 2 })).to.equal(
          "Promise{…}",
        );
        expect(inspect(Promise.resolve(), { truncate: 1 })).to.equal(
          "Promise{…}",
        );
      });
    });
