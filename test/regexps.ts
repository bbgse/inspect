...
describe("regexps", () => {
  ...
  describe("colors", () => {
    it("returns string with red color, if colour is set to true", () => {
      expect(inspect(/abc/, { colors: true })).to.equal(
        "\u001b[31m/abc/\u001b[39m",
      );
    });
  });
  ...
  describe("truncate", () => {
    it("returns the full representation when truncate is over string length", () => {
      expect(inspect(/foobarbaz/gim, { truncate: 14 })).to.equal(
        "/foobarbaz/gim",
      );
    });

    it("truncates strings longer than truncate (13)", () => {
      expect(inspect(/foobarbaz/gim, { truncate: 13 })).to.equal(
        "/foobarb…/gim",
      );
    });

    it("truncates strings longer than truncate (12)", () => {
      expect(inspect(/foobarbaz/gim, { truncate: 12 })).to.equal(
        "/foobar…/gim",
      );
    });
