...
export function inspectAttribute(
  ...
  return `${options.stylize(String(key), "yellow")}=${options.stylize(
    `"${value}"`,
    "string",
  )}`;
}
...
export function inspectHTMLCollection(
...
// @ts-ignore (Deno doesn't have Element)
export default function inspectHTML(
  element: Element,
  options: Options,
): string {
